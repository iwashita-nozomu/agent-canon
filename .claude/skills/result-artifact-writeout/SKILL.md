...
1. For cross-run retention of a run-local agent report, do not hand-generate an archive report. Use `python3 tools/agent_tools/runtime_log_archive_git.py archive-agent-report --report-dir reports/agents/<run-id>` followed by `runtime_log_archive_git.py push`; the tool writes `.agent-canon/log-archive/agent-reports/<repo-key>/<run-id>/<snapshot-id>/`, `archive_manifest.json`, and `index.jsonl`.
