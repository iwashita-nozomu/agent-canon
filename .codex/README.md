...
- `PreToolUse` の critical `hooks/branch_worktree_guard.py` は、同じ checkout を複数 chat/session が共有する前提で `git restore/reset/clean/checkout/switch/stash` と branch/worktree mutation を shell 実行前に分類します。unknown dirt は user/other-chat owned とし、proven exact task ownership は approval request の path を限定するだけで destructive approval を迂回しません。destructive intent は同一 command segment の `AGENT_CANON_DESTRUCTIVE_GIT_AUTHORITY=explicit_user_approval` と非空 `AGENT_CANON_DESTRUCTIVE_GIT_REASON`、通常の branch/worktree creation は既存の route authority と reason が揃う場合だけ通します。child の launch error、nonzero、timeout、nonempty malformed output は fail-closed、empty stdout は allow です。session 開始時に dispatcher と child registration を load 済みなら、次の tool call は更新後 child script を実行する hot path です。一方、hook table 自体を load していない既存 session には後付けできず、保護を有効にするには session restart が必要です。
