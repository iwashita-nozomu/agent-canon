...
// upstream fixture ../../../tests/fixtures/python_algorithm_contract/ named fixture rows
// @dependency-end
...
fn valid_fixture_has_canonical_json_projection() {
    let path = fixture_root("valid.py");
    let output = run_cli(&[
...
fn protocol_only_imports_have_one_non_allowlisted_finding_and_allowlisted_zero() {
    let root = fixture_root("protocol");
    let output = run_cli(&[
        "python-algorithm-contract-check",
        "--root",
        root.to_str().unwrap(),
...
fn public_surface_and_info_findings_use_canonical_kinds() {
    let source = fs::read_to_string(fixture_root("valid.py")).expect("valid fixture reads");
