...
責任単位として固定します。Reader Map 後の semantic section は、実際の見出しと
同じ次の順序で読みます。

1. `Design Status`
1. `Request Clauses`
1. `Abstract Design Frame`
1. `Validation ID Definitions`
1. `Design Side-Effect Summary`
1. `Branch, Merge, and Scope Evidence`
1. `Installed and Existing Implementation Reuse Survey`
1. `Durable Post-Merge Predecessor Integration Record`
1. `Normative Typed Value`
1. `Normative agents_config.json Record`
1. `Reference Grammar and Resolution Algorithms`
1. `Public Types and APIs`
1. `Validate-All, Render-All, Atomic Call Graph`
1. `Atomic Publication and Failure Contract`
1. `Producer Call Paths and Public Behavior`
1. `Waterfall Manifest-Consumer Ownership`
1. `Historical Additions Deleted or Replaced`
1. `File Scope and Individual Validation Mapping`
1. `Implementation Source Packet`
1. `Detailed Design Side-Effect Map`
1. `Design-To-Implementation Trace`
1. `One-Writer Implementation Sequence`
...
schedule projection in both bound packet files, the V11 five-file identity
...
| --- | --- | ---: | --- |
| `active_design_packet_implementation_surface_route.txt` | `a85e3540e3bba37b03b494be2779673c156afb66b3a1495aed3b4fdb5f5d1a1b` | 220 | exclusive to `active-design-packet-materialization` |
| `active_design_packet_implementation_request.txt` | `87b495f6521cfd507864c3a88a31cdb2a44cf9fbfd3ff049b46670dd50060e1c` | 222 | exclusive to `active-design-packet-materialization`; binds the route SHA and line count |
| this design | computed after final formatting and checks | 3792 | binds both packet SHA/line-count pairs |
...
## Validation ID Definitions

These predicates are defined once here, before their first normative use. The
later `Targeted Validation` section supplies execution commands and does not
redefine them.
...
| V11 | external overlap closure | all five remaining external hashes and line identities match; productive-wait remains non-authoritative until separately approved; static symbol/section comparison shows active packet owns packet blocks/pure renderer/producer activation while WAIT-1 owns only the later reservations listed in the precedence section; integration order is verified `knowledge_graph` plus `active_design_packet_materialization` records at one OID → approved WAIT-1 if any, with no concurrent edit, aggregate, or schema bridge |
...
| S10 | external overlap | five hash-bound productive-wait inputs are verify-only; the two required records integrate at one source OID first and freeze exact predecessor blocks before any narrower successor edit |
...
The five remaining productive-wait files and hashes in Design Status are the
complete overlap input set. They are verify-only evidence, not forbidden
unknowns. Their status is decisive: the productive-wait design is an
unapproved v17 interim candidate with historical REVISE/ESCALATE reviews and
cannot authorize a concurrent edit to this unit or waive any predecessor gate.
...
2. After that source PR merges, invoke the generic producer once for
   ...
   record or one of the five external files may substitute for either record.
...
then precede WAIT-1 if approved.
...
Forbidden scope is `.github/workflows/**`, all edits to the five hash-bound
...
2. `active_design_packet_implementation_surface_route.txt`, SHA-256
   `a85e3540e3bba37b03b494be2779673c156afb66b3a1495aed3b4fdb5f5d1a1b`,
   220 lines;
3. `active_design_packet_implementation_request.txt`, SHA-256
   `87b495f6521cfd507864c3a88a31cdb2a44cf9fbfd3ff049b46670dd50060e1c`,
   222 lines; its header and metadata bind the route SHA and line count;
...
and the five productive-wait overlap files. Their hashes and
...
3. the five external overlap files, verifying their hashes, status, exact
...
| S10 | ADP-04, ADP-09, ADP-13 | five hash-bound productive-wait overlap inputs are verify-only; one Luna worker executes the graph-owned dirty semantic intent, active packet, affected source/config, skills/docs, generated views, and checker cleanup as one owner-preserving responsibility graph in dependency order; both required predecessor records for the merged source OID integrate first, followed by optional approved WAIT-1; split occurs only for an actual concurrent write conflict or unresolved predecessor | current graph-owned dirty paths and productive-wait reconciliation/design/reviews | graph predecessor paths under graph authority, `agent_team.py`, workflow monitor, task-start/bootstrap, manifest/docs/skill/generated-view sections, and post-merge record/graph evidence | one Luna writer preserves each node's path authority, lands the integrated source PR, and hands off one V1–V12 evidence set; later units consume both archived records and narrower reservations | equal-SHA reviews plus change/final review check owner boundaries, hashes, reservations, and one-worker handoff; successor verifies both records and common OID | V11, V12 |
...
| T10 | ADP-04, ADP-09, ADP-13 | exact remaining external hashes, statuses, shared-symbol reservations, two-record predecessor requirement, and one-Luna owner-preserving dependency order; no file/finding/test/review slice is valid | current graph-owned dirty state and five productive-wait overlap artifacts | one responsibility-graph handoff, graph paths under graph authority, frozen active-packet blocks, explicit predecessor set, and post-merge graph validation | S10 | V11, V12 |
...
5. Run V1-V9 and V11 before merge. Then merge the one source PR, produce,
   archive, and individually and jointly verify both predecessor records at the
   merged source OID, and record that post-merge transition as V10. Run V12 only
   after V10 succeeds. Change and final review receive the one Luna diff, one
   responsibility-graph trace, and one V1-V12 handoff; generated views and
   checker disposition are not separate review slices. This is the normative
   state order; historical source work that predates `ORDER-REPAIR-01` remains
   historical evidence and is not retroactively authorized by this repaired
   ordering.
...
The normative V1-V12 predicates are defined in
`Validation ID Definitions`, before their first normative use. This section
only binds those predicates to executable commands and evidence order.
...
V11's normative source workflow runs before source editing and again in change
review. `ORDER-REPAIR-01` executes these identity predicates after historical
source work only as current evidence; that execution does not retroactively
satisfy or claim a pre-source authorization gate.

```bash
sha256sum --check <<'V11_SHA256_IDENTITIES'
14e706c816d3deb76bb60d3533800429c6cfd6dcfa83f1f418d935efcbbb9e5f  reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_reconciliation_v1.md
c3701aab5c86898d2990089426cb0af0c662282178bf95d10691fc2a0f5835e7  reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_design.md
4caaec7250bab1615d3f6cc6374e64b9d87be38f75d8bb7d513754059117a2f7  reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_detailed_design_review_v14.md
ed25086f1181291ace326d3f73dbbbdbe32d5fbb56a50a42c8c56156fff7f3fe  reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_detailed_design_review_v16.md
1a6fe63239749a8dba45880498f92ac2c640c7949f40617462c72bee7b598fae  reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_review_v1.md
V11_SHA256_IDENTITIES

while IFS=' ' read -r expected_lines path; do
  actual_lines="$(wc -l < "$path")"
  test "$actual_lines" -eq "$expected_lines" || exit 1
done <<'V11_LINE_IDENTITIES'
101 reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_reconciliation_v1.md
453 reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_design.md
231 reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_detailed_design_review_v14.md
226 reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_detailed_design_review_v16.md
239 reports/agents/20260712-090608-context-packettool-skill-routing/productive_wait_contract_review_v1.md
V11_LINE_IDENTITIES
...
The V11 identity universe is exactly the same five explicit paths in the hash
and line commands above. Their expected hashes are the five matching Design
Status values, and their expected line identities are encoded in the second
command. The five-file cardinality and line counts are identity predicates
only; they never define scope authority. Review maps every symbol hit to the
reservations in `External overlap inputs, precedence, and integration order`;
an unclassified or cross-owned hit fails V11. This is a symbol/section
...
| productive-wait overlap remains verify-only and cannot run concurrently with this unit | five recorded hashes/statuses and exact shared-symbol reservations | observed external evidence plus fixed integration decision |
...
the exact five-file V11 identity predicates, V12 typed-binding-to-literal-argv
