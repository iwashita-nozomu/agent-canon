...
一度に検証し、選択された record を新しい run manifest へコピーします。role の
`required_outputs`、write scope、reader packet は同じ選択 record を参照し、生成後は
manifest record が run authority になります。
...
- `durable_user_preference`
  - private `agent-canon-log/knowledge/` や過去ログから抽出された user tendency
...
ルール:
- 不明点はすぐユーザーへ戻さず、まず `documents/`、private `agent-canon-log/knowledge/`、`documents/notes/themes/`、`documents/notes/guardrails/`、`documents/notes/knowledge/`、`documents/notes/failures/`、prior logs、local code / tests から解決を試みます
...
- AgentCanon tool image を変える pass では bootstrap container contract、実 `bootstrap.sh install/start`、tool/eval smoke、exact resource absence readback を validation plan に含めます。project Docker は project-owned validation route に委譲します
