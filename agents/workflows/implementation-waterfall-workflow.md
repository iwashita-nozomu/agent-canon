...
- 包括的開発では、same directory / same public API surface の parallel write を許可しません。独立した source workstream は、substantial replaceable responsibility unit、computed workspace clone、disjoint write scope、dependency/merge order、validation route、reviewer ownership を持ち、かつ vendor checkout が別の active topic/branch に占有されている場合だけ `--placement workspace` で分離できます。parallel eligibility だけでは clone を作成しません。fresh route は既存 local/remote branch を拒否し、継続は `--placement workspace-continuation` に分けます。vendor-first は非並列 single-stream の既定であり、ready な全 stream を launch した parent は descendants を monitor し、同一責任・同一 context を再利用します。
