...
AgentCanon source repository の standard test route は例外なく次です。

```bash
cd <agent-canon-git-root>
docker build -f docker/Dockerfile -t <task-owned-tag> .
docker run --rm <task-owned-tag> testrunner.sh
```

parent repository は AgentCanon Git root へ移動してこの entrypoint を呼ぶだけにし、
`test/testlist.toml` の command、internal test path、fixture、mount を知りません。runner は
各 command の `argv`、`environment = product|tooling`、`code_owner`、
`responsibility_scope`、exit code を出力します。failure を修正するときはこの metadata で
environment と code owner を分けます。使用後は task-owned container と image の exact ID
を削除して absence を確認し、shared image や system-wide prune は対象にしません。
...
- AgentCanon source acceptance では追加 mount、parent側 test列挙、runtime install、追加
  container environmentを渡さず、`testrunner.sh` だけを呼びます。
