...
1. 原則として `agent-runtime-compact.md` だけを読み、machine summary、priority problems、next actions、selection misses を分析します。
1. Raw JSONL を読むのは、compact summary の `evidence` に出た個別 path を確認する場合だけです。その場合も `tail`、小さい parser、または path 限定 `rg -n` を使い、全ログ横断の一致行 dump を避けます。
...
- Full dashboard は human review 用です。agent の通常分析入力は compact summary を既定にします。
