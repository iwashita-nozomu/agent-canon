...
1. IssueWorker の publisher は GitHub mutation 前に external runtime/spool の receipt route を確認します。成功後は resident container の `issue_sync.py --stage-publication-receipt` で body-free receipt を spool へ書き、host shell の private-log sync 後に dashboard が published namespace を読みます。route がない成功や pending 消失から公開済みとは推定しません。
