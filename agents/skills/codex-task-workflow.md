...
1. When AgentCanon source, submodule pin, `.gitmodules`, AgentCanon-owned root runtime view, root-copy surface, or parent root sync changes, record `agentcanon_structure_followup=required`. Use the source-root resolver `link-root` and `check` commands in the template / derived parent root, then record `agentcanon_structure_followup=pass` only after both commands pass.
