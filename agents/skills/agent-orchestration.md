...
1. prompt-derived skill routing が必要なら `python3 tools/agent_tools/route.py --prompt "<user request>" --format json` を使い、`ACTIVE_SKILLS` を current stage の宣言、`DEFERRED_SKILLS` を後続 wave trigger として扱う。`tools/bin/agent-canon local-llm route-skill ...` は同じ catalog-backed route の CLI 互換 mirror です
