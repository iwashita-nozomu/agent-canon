...
1. slice、decomposition、prototype、または worker handoff の前に、involved Git roots / modules が二つ以上か、module の consumer が dependency repository にあるかを先に判定する。条件を満たしたら、deferred / candidate の `$dependency-analysis` を ACTIVE に昇格・起動してから decomposition、prototype、worker handoff に進み、既存 packet の dependency DAG、scope、repair batches、reuse survey、validation route、allowed paths / handoff context を全 child に同じ内容で relay する。単一 module で dependency-repository consumer がない場合は既存 fast pathを使い、これは新しい承認・artifact・machine gateではない
