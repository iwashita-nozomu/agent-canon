...
1. slice、decomposition、prototype、または worker handoff の前に、involved Git roots / modules が二つ以上か、module の consumer が dependency repository にあるかを先に判定する。該当時だけ `$dependency-analysis` を起動し、既存 packet の dependency DAG、scope、repair batches、reuse survey、validation route、allowed paths / handoff context を全 child に同じ内容で relay する。単一 module で dependency-repository consumer がない場合は既存 fast pathを使い、これは新しい承認・artifact・machine gateではない
