...
four-entry `waterfall.design_packet.v1` record. Its selected design, technical
review, and document-flow review paths are projected into one run bundle and
persisted at `team_manifest.yaml#run.active_design_packet`. The four entries
are projected into the matching sections of `design_brief.md`;
...
the packet, validates its closed field set, relative artifact paths, and closed
reference fields, renders all selected templates in memory, and publishes one
complete bundle for task-start, bootstrap, and document-start producers. Review
templates are authorization records only and cannot write or advance the active
run pointer.
