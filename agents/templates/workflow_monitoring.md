...
<!-- Mirror schedule.md Agent Wave Ledger events as concise `wave_event=...` token rows so dynamic expansion is searchable. Required tokens for each row: wave_id, event_kind, trigger, budget_before, budget_after, spawned_roles, skipped_roles, rationale, status. -->
