...
- closeout 前に、untracked な report file が `reports/agents/<run-id>/`
  以外へ残っていないことを確認します。run bundle 外の未追跡 report は
  `runtime_log_archive_git.py sync` の収集対象外なので、current run の
