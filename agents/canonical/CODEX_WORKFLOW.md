...
- user が goal-driven intent を示したが exact `/goal <objective>` を渡していない場合は、parent が target-state-complete Objective を作り、`goal.md` に先に固定します。未確定の intake draft は read-only discovery であり、編集を認可しません。
...
- active frontier、write scope、nested reservation、queue は capacity handshake owner が宣言 topology から生成します。固定 active/write 数、task-size/count/time budget、または universal ceiling claim は workflow policy ではありません。fixed packet の標準経路は one Spark と one post-completion owning gate で、Luna は ambiguous design、causal repair、graph-owned cross-owner integration、review を保持します。
