...
- `agent-eval-accumulation`
  - missing / stale な AgentCanon eval family を registered producer と checker で append-only evidence に戻す
- `agent-canon-update`
...
- `algorithm-proof-exploration`
  - 証明義務を入力に JIT-canonical IR / theorem graph / algorithmic blocker frontier でアルゴリズム選択と必要な algorithm change を探索し、formal-proof へ渡す
- `algorithm-flowchart`
  - JIT-canonical IR / theorem graph / proof_status を Mermaid block chart に射影し、実装されている反復法と証明状態を機械的に可視化する
- `research-workflow`
