...
- Codex の default は、文書・計画・research・review と broad / ambiguous implementation を `gpt-5.5` `high`、code survey / static test design / language-specific code review を `gpt-5.3-codex-spark` `minimal`、design-traced narrow implementation slice を `gpt-5.3-codex-spark` `minimal` に分ける
...
| Codebase Survey / Test Design / Language-Specific Code Review | `explorer`, `test_designer`, `python_reviewer`, `cpp_reviewer` | `gpt-5.3-codex-spark` | `minimal` |
| Low-Latency Narrow Implementation | `spark_worker` | `gpt-5.3-codex-spark` | `minimal` |
...
- この repo ではそれに合わせて、文書・計画・research・review と broad / ambiguous implementation を `gpt-5.5`、狭い code survey / static test design / language review を `gpt-5.3-codex-spark` `minimal`、設計済み低リスク実装 slice を `gpt-5.3-codex-spark` `minimal` に寄せます。
