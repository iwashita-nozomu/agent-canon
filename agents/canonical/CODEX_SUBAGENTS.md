...
- 独立 source workstream を選択する parent packet は、各 stream の substantial replaceable responsibility unit、`dependency_module_change.py` typed route、disjoint write scope、dependency/merge order、validation route、reviewer ownership を固定します。vendor が別 topic/branch に occupied の場合のみ、fresh な `dependency_module_change.py prepare --placement workspace` を使います（`local/remote` branch collision は拒否、継続は `--placement workspace-continuation`）。vendor が同一 topic 時は vendor branch で source owner を維持し、dirty / ahead / diverged は evidence として保持します。ready な非衝突 stream は全て launch し、parent / delegated stage owner は全 descendants を monitor します。
