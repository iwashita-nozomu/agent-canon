...
class SkillShimMaterializerTest(unittest.TestCase):
    ...
    def test_image_build_mode_uses_the_same_materializer(self) -> None:
        """The Docker build mode writes the same catalog-derived target bytes."""
        with patch.dict(
            "os.environ", {"AGENT_CANON_IMAGE_BUILD": "1"}
        ):
            actual = materialize(PROJECT_ROOT, all_skills=True, image_build=True)
        self.assertEqual(actual["status"], "pass")
        self.assertEqual(actual["content_delta_count"], 0)
