"""Regression tests for canonical Docker GPU CDI documentation."""

# @dependency-start
# contract test
# responsibility Tests CDI device injection, exact environment forwarding, and environment-skill alignment for GPU containers.
...
class GpuExecutionCdiContractTest(unittest.TestCase):
    """Keep Docker GPU execution on one CDI and full-UUID contract."""
    ...
    def test_gpu_skill_declares_cdi_device_and_exact_environment(self) -> None:
        self.assertIn("--device nvidia.com/gpu=all", self.gpu_text)
        self.assertIn("selected full UUID", self.gpu_text)
        for name in GPU_ENVIRONMENT_NAMES:
            with self.subTest(name=name):
                self.assertIn(f"-e {name}", self.gpu_text)

    def test_wsl_all_device_keeps_compute_visibility_on_full_uuid(self) -> None:
        self.assertIn("WSL", self.gpu_text)
        self.assertIn("nvidia.com/gpu=all", self.gpu_text)
    ...
    def test_contract_does_not_restore_legacy_or_cgroup_bypass_examples(self) -> None:
        ...
        self.assertNotIn("docker run --gpus", combined)
    ...
    def test_environment_owner_delegates_gpu_wiring_to_gpu_execution(self) -> None:
        ...
        self.assertIn(
            "docker run --device nvidia.com/gpu=<qualified-device>",
