...
# upstream implementation ../../tools/agent_tools/check_tool_convention_drift.py checker
...
CHECKER = PROJECT_ROOT / "tools" / "agent_tools" / "check_tool_convention_drift.py"
...
class CheckToolConventionDriftTest(unittest.TestCase):
            ...
            tool.write_text(
                "\n".join(
                    [
                        ...
                        "# upstream implementation ./check_tool_convention_drift.py drift gate",
                        "# @dependency-end",
                        "",
                    ]
                ),
    ...
    def test_reverse_required_link_fails_when_only_direct_edge_exists(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn(
                ...
                "tools/agent_tools/check_tool_catalog.py:tools/catalog.yaml",
                result.stdout,
            )
    ...
    def test_catalog_legacy_default_confusion_fails(self) -> None:
        """Legacy provenance entries cannot become default callable tools."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            self.write_tool_catalog_contract(root)
            catalog = root / "tools" / "catalog.yaml"
            ...
            self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
            self.assertIn(
                "legacy-default-confusion:tool_catalog:"
                "tools/legacy/example:callable-by-default",
                result.stdout,
            )
    ...
    def write_tool_catalog_contract(self, root: Path) -> None:
        ...
        self.write_file(
            ...
            "\n".join(
                [
                    ...
                    "# downstream implementation ../../tests/agent_tools/test_check_tool_catalog.py tests",
                    "# @dependency-end",
                    "",
                ]
            ),
        ...
        self.write_file(
            ...
            "\n".join(
                [
                    "# @dependency-start",
                    "# responsibility Defines fixture tool catalog.",
                    "# downstream implementation agent_tools/check_tool_catalog.py checker",
                    "# @dependency-end",
                    "",
                    "version: 1",
                    "entries:",
        ...
        for relative in [
            ...
            "tools/legacy/jax_solver_util/README.md",
            "tools/ci/run_all_checks.sh",
            ...
            self.write_file(
                ...
                "\n".join(
                    [
                        ...
                        "upstream design README.md fixture anchor",
                        "downstream implementation tools/agent_tools/check_tool_catalog.py checker",
                        "@dependency-end",
                        "-->",
                        "tools/catalog.yaml",
