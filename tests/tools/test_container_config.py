...
# upstream implementation ../../test/testrunner.sh provides the source test entrypoint
# @dependency-end
...
def test_standalone_image_context_is_explicit_and_source_owned() -> None:
    """The public source image owns tests without activating the legacy allowlist."""
    module = load_container_config_module()

    assert module.is_public_test_source(PROJECT_ROOT)
    assert module.validate_dockerignore(PROJECT_ROOT) == []
    dockerfile = (PROJECT_ROOT / "docker" / "Dockerfile").read_text(encoding="utf-8")
    assert "COPY . /opt/agent-canon-parent/vendor/agent-canon/" in dockerfile
    assert 'CMD ["testrunner.sh"]' in dockerfile
...
def test_post_create_uses_shared_lifecycle() -> None:
    """The default and explicit GPU selectors use the shared lifecycle."""
    for config_path in (
...
def test_generator_scenarios_require_default_and_gpu_compose_outputs(
    tmp_path: Path,
) -> None:
    """A generator that exits successfully without output fails selected scenarios."""
    repo = write_topic_fixture(tmp_path)
...
def test_default_project_identity_requires_update_remote_user_uid_false(
    ...
    config_path = repo / ".devcontainer/devcontainer.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    config["updateRemoteUserUID"] = True
    config_path.write_text(json.dumps(config), encoding="utf-8")
