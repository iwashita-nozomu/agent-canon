...
class MirrorSkillShimsTest(unittest.TestCase):
    ...
    def test_check_reports_pending_changes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.write_file(source / "skill-a" / "SKILL.md", "source skill a\n")
            self.write_file(source / "skill-a" / "references" / "guide.md", "guide v2\n")
            self.write_file(source / "skill-b" / "SKILL.md", "source skill b\n")

            self.write_file(target / "skill-a" / "SKILL.md", "stale skill a\n")
            self.write_file(target / "skill-a" / "stale.txt", "remove me\n")
            self.write_file(target / "skill-old" / "SKILL.md", "old skill\n")
