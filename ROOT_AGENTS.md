...
- Existing branch / PR reuse is the default. If the current branch or PR already owns the same task, added user instruction, shared-canon surface, or small follow-up, continue it instead of creating a new branch. Do not create a branch just to start fresh, avoid dirty state, split a checklist addendum, or react to a mid-task user instruction. A new branch requires a recorded reason such as merged / closed / unpushable current branch, separate ownership lane, explicit review isolation, protected-surface conflict, unsafe divergent state, or explicit user request. Record `branch_creation_reason=<reason>` in the run bundle, work log, or PR body before creating it.
