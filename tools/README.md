...
- `agent_tools/`
  ...
  - `helper_function_inventory.py` は Python helper 関数を AST/call graph/side effect facts から列挙し、呼び出し元と専用ヘルパ化の疑いを JSON / Markdown / text で出します。
...
- `helper_function_inventory.py` inventories Python helper functions and infers roles from AST body facts, calls, side effects, path context, internal call graph evidence, and single-caller or file-local specialization.
