...
- `agent_tools/`
  ...
  - `tool_rejection_preflight.py` は planned edit path から OOP readability、helper inventory、dependency review、GitHub workflow、log-surface inventory などの予測 reject gate を出し、write-capable subagent handoff に渡す `TOOL_REJECTION_PREDICTED_GATE` 行を生成します。
