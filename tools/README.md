...
- `agent_tools/`
  ...
  - `lean_proof_env.py` は Mathlib / Aesop を含む Lean 4 Lake 環境を AgentCanon 側に作り、`smoke` または `check-file` で generated proof stub を検査します。active theorem package では Mathlib / Aesop を一度 pin して `lake build` で再利用し、この tool は探索用・fallback 用の環境確認に使います。
