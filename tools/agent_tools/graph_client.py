...
GRAPH_RELATIONS = frozenset({"all", "dependency", "owner", "scope", "import", "include", "symbol", "call", "containment", "document", "catalog", "pin", "view", "generated", "submodule", "public"})
...
class GraphClientError(RuntimeError):
    """Canonical graph invocation or response validation failed."""


@dataclass(frozen=True)
class GraphSourceIdentity:
    """Exact source identity returned by graph context."""
...
class GraphDependencyFact:
    """One explicit dependency declaration projected from graph rows."""
...
@dataclass(frozen=True)
class GraphResponse:
    """One command-specific graph response, including nonzero freshness states."""
    ...
    def source_identity(self) -> GraphSourceIdentity | None:
        """Return the graph-certified source identity without re-reading source files."""
        ...
        if len(content) != 64 or any(character not in "0123456789abcdef" for character in content):
            raise GraphClientError("graph context source identity hash is invalid")
        return GraphSourceIdentity(cast(str, identity["snapshot_commit"]), cast(str, identity["source_path"]), content)

    @property
    def dependency_facts(self) -> tuple[GraphDependencyFact, ...]:
        """Project explicit dependency facts using only graph node identities."""
        ...
        for raw_node in raw_nodes:
            ...
            if not isinstance(node_id, str) or not isinstance(endpoint, str) or not endpoint:
        ...
        for raw_fact in raw_facts:
            ...
            detail = _required_mapping(fact.get("dependency_detail"), f"graph fact {fact_id}.dependency_detail")
            projected.append(GraphDependencyFact(
                fact_id,
                _required_string(detail, "direction", "dependency detail"),
                _required_string(detail, "kind", "dependency detail"),
                paths[source_id], paths[target_id], _required_string(detail, "reason", "dependency detail"),
                _required_string(fact, "producer", fact_id), _optional_string(fact.get("source_path")),
                _optional_mapping(fact.get("source_span")), _required_string(fact, "evidence_ref", fact_id),
                _required_string(fact, "authority", fact_id),
            ))
        return tuple(sorted(projected, key=lambda item: (item.source, item.target, item.direction, item.kind, item.id)))


class GraphClient:
    """Invoke and validate the one canonical graph command surface."""

    def __init__(self, root: Path, executable: Path | None = None) -> None:
        """Bind one repository root and canonical graph executable."""
    ...
    def invoke(self, command: str, options: Sequence[str] = ()) -> GraphResponse:
        """Invoke one graph command and consume its typed JSON response."""
        if command not in GRAPH_COMMANDS:
            raise GraphClientError(f"unsupported graph command: {command}")
        argv = [str(self.executable), "graph", command, "--root", str(self.root), "--format", "json", *options]
        try:
        ...
        if not all(isinstance(value, str) and value for value in (schema, response_command, status)):
    ...
    def context(self, path: str, token: str | None = None) -> GraphResponse:
        """Read one persisted graph context response."""
        options = ["--path", path]
        if token is not None:
            options.extend(("--token", token))
        return self.invoke("context", options)
