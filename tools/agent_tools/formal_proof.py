...
def build_verification_commands(stub_path: str, target: str) -> tuple[str, ...]:
    ...
    if target == "lean":
        return (f"lake env lean {stub_path}",)
    if target == "isabelle":
...
def build_plan(
    ...
    return FormalProofPlan(
        ...
        proof_obligations=build_obligations(sections),
...
def main() -> int:
    ...
    plan = build_plan(claim_text, args.target, domains, args.name, stub_path=None)
    if args.out_dir:
        plan = write_outputs(plan, Path(args.out_dir), args.name)
