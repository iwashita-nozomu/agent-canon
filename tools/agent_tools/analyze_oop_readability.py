...
def is_supported_source(path: Path) -> bool:
    ...
    return path.suffix in PYTHON_SUFFIXES | CPP_SUFFIXES


def visible_source_path(root: Path, path: Path, exclude_patterns: list[str]) -> bool:
...
def iter_directory_sources(root: Path, target: Path, exclude_patterns: list[str]) -> list[Path]:
    """Return supported files below one directory target."""
    return [
        path.resolve()
        for path in sorted(target.rglob("*"))
        if visible_source_path(root, path, exclude_patterns)
    ]
...
def iter_source_files(root: Path, raw_paths: list[str], exclude_patterns: list[str]) -> list[Path]:
    ...
    for target in source_targets(root, raw_paths):
        if is_excluded_path(root, target, exclude_patterns):
            continue
        if target.is_file() and is_supported_source(target):
            files.append(target.resolve())
            continue
        if target.is_dir():
            files.extend(iter_directory_sources(root, target, exclude_patterns))
