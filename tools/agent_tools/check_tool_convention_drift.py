...
# upstream implementation ./check_tool_catalog.py validates catalog structure
# downstream implementation ../../tools/ci/run_all_checks.sh runs drift checker
# downstream implementation ../../tests/agent_tools/test_check_tool_convention_drift.py tests checker
# @dependency-end
...
CONTRACTS = (
    ...
    ToolContract(
        name="tool_catalog",
        tool="tools/agent_tools/check_tool_catalog.py",
        links=(
            ...
            LinkCheck("tools/legacy/jax_solver_util/README.md"),
            LinkCheck("tools/ci/run_all_checks.sh"),
            LinkCheck("tests/agent_tools/test_check_tool_catalog.py"),
        ),
        text_checks=(
            TextCheck(
                "tools/README.md",
    ...
    ToolContract(
        ...
        links=(
            ...
            LinkCheck("tools/ci/run_all_checks.sh"),
            LinkCheck("tools/agent_tools/check_tool_convention_drift.py"),
        ),
    ),
    ToolContract(
...
def check_catalog_entries(root: Path) -> list[Finding]:
    ...
    for index, raw_entry in enumerate(entries, start=1):
        ...
        if entry_path.startswith("tools/legacy/") and status != "legacy_provenance":
            findings.append(
                Finding(
                    "legacy-default-confusion",
                    "tool_catalog",
                    entry_path,
                    "legacy-path-not-provenance",
                )
            )
        if status == "legacy_provenance":
            ...
            if wiring.get("ci") is True or wiring.get("pr_check") is True:
                findings.append(
                    Finding(
                        "legacy-default-confusion",
                        "tool_catalog",
                        entry_path,
                        "default-wired-legacy",
                    )
                )
    return findings
