...
jq -r '
  (.nodes | map({key: .id, value: (.path // .selector)}) | from_entries) as $paths
  | .facts[]
  | select(.kind == "dependency" and .inferred == false)
  | [(.dependency_detail.direction // ""), (.dependency_detail.kind // ""), ($paths[.from] // ""), ($paths[.to] // "")]
  | select(.[0] != "" and .[1] != "" and .[2] != "" and .[3] != "")
  | @tsv
