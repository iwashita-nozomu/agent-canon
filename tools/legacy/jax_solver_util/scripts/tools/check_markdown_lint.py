...
def iter_markdown_lines(filepath: str):
    """Yield markdown lines with code-fence and display-math state."""
    with open(filepath, encoding="utf-8", errors="ignore") as f:
        lines = f.readlines()
    ...
    for i, line in enumerate(lines, 1):
        ...
        if stripped.startswith("```"):
            ...
            yield i, line, in_fence, in_display_math
            continue
...
class MarkdownLinter:
    ...
    def check_md004(self, filepath: str) -> list[tuple[int, str]]:
        ...
        if style == "consistent":
            ...
            for i, line, in_fence, in_display_math in iter_markdown_lines(filepath):
                if in_fence or in_display_math:
                    continue
                match = re.match(r"^( *)[-*+]\s", line)
                if match:
                    depth = len(match.group(1)) // 2
                    marker_index = match.start(1) + len(match.group(1))
                    marker = "-" if match.group(0)[marker_index] == "-" else "*"
    ...
    def scan_files(self, file_patterns: list[str]) -> dict[str, list[tuple[int, str, str]]]:
        ...
        ignored_parts = [".git", ".worktrees", "__pycache__", "Archive"]
        md_files = [str(f) for f in md_files if not any(part in str(f) for part in ignored_parts)]
