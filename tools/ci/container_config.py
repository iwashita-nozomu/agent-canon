...
# @dependency-start
# contract tool
# responsibility Validates Dockerfile, runtime pack, explicit rootful/rootless devcontainer identities, and retired VS Code projection paths.
...
def load_devcontainer_json(
    ...
    *,
    ...
    relative = relative or ".devcontainer/devcontainer.json"
    try:
...
def validate_devcontainer_json(
    ...
    *,
    ...
    workspace_placeholder = "$" + "{localWorkspaceFolderBasename}"
    compose_output = (
    ...
    expected_name = (
        f"{workspace_placeholder}-devcontainer"
        if identity_mode == "project"
        else f"{workspace_placeholder}-rootless-devcontainer"
    )
    ...
    expected_json = {
        ...
        "workspaceFolder": f"/workspace/{workspace_placeholder}",
...
def validate_generated_compose(
    ...
    *,
    ...
    expected_runtime_user = "0:0" if identity_mode == "rootless-root" else None
    if identity_mode == "rootless-root" and service_user != expected_runtime_user:
        findings.append(
            Finding(
                "dependency_contract_violation",
                relative,
                "rootless-runtime-user-must-be-0:0",
            )
        )
    if identity_mode == "project" and service_user is not None and (
    ...
    required_environment = {
        ...
        "AGENT_CANON_CONTAINER_USER": (
            "project" if identity_mode == "project" else "root"
        ),
    }
    required_environment["SHELL"] = pack.shell if pack is not None else "/bin/bash"
