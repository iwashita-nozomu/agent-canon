...
`AGENT_CANON_LEAN_TOOLCHAIN=leanprover/lean4:v4.30.0` by default, so this
helper can run `lake update`, `lake build`, and `lake env lean` in fresh
containers without host-global Lean setup.
