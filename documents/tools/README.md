...
- `tools/experiments/save_experiment_result_annex.py`
  - source checkout で作成した `experiments/<topic>/result/<variant>/<run_name>/` と optional report を、独立 annex worktree の一つの deterministic archive に保存します。
  - 標準形は `python3 -m tools.experiments.save_experiment_result_annex --result-dir experiments/<topic>/result/<variant>/<run_name> --annex-repo "$EXPERIMENT_RESULT_ANNEX_REPO"` です。archive target は `experiments/<topic>/result/<variant>/<run_name>.tar.gz` です。
