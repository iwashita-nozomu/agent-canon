...
- `tools/agent_tools/evaluate_skill_workflow_prompts.py`
  ...
  - hook JSONL は `git@github.com:iwashita-nozomu/agent-canon-log.git` を `.agent-canon/log-archive/` に mount して蓄積します。branch / push 手順は `documents/runtime-log-archive.md` を正本にし、通常操作は `tools/agent_tools/runtime_log_archive_git.py ensure|status|import-legacy|push` を使います。
  - `generate_agent_improvement_guide.py` は `memory/`、`agents/evals/results/skill-workflow-prompt/`、mounted hook archive、legacy `agents/evals/results/hook-runs/`、`issues/open|closed/` を読んで PR / branch push 用の改善指南書を生成します。生成は read-only で、skill usage、hook event、tool name、checker target、protocol feedback token の不足をまとめ、実修正は local Agent / Copilot PR に渡します。
