...
`.agent-canon/knowledge-graph/graph.sqlite`. `status`, `query`, and `context`
are read-only; they never rebuild or fall back to a header scan.
...
non-authorizing observation projection: its immutable producer ID/version is
an input-freshness term, while its exact captured payload hash remains in the
producer artifact and `graph_fingerprint`. New hook activity therefore does
not make graph observation self-invalidating, and context still returns the
exact runtime snapshot bound to that graph fingerprint. Persisted logical
records are rehashed on status; modifying a node, relation, diagnostic,
producer record, or mathematical witness yields `invalid`.
