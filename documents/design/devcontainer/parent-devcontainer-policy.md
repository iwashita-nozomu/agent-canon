...
| DEV-DEFAULT-012 | project Docker runtime、parent devcontainer、AgentCanon Codex tooling を別 owner として解決し、pack/rules/profile override がない direct route は project `docker/Dockerfile` と AgentCanon source-relative profile default を使う | parent project は `docker/packs/`、`docker/codex-container-profiles.toml`、`docker/python-execution-rules.toml` を必須 surface にしない。nested Codex の既定 HOME は tracked tree へ credential を残さない parent-owned ignored `workspace/.nested-codex/<profile>` とし、host `.git-credentials` projection は既定で無効にする。存在する project pack/rules と明示 `--profiles` だけを optional override として検証・消費する | packless parent の `container_config.py`、stable repo-marker root resolution、AgentCanon profile path、安全な HOME/credential defaults、rules/pack absent の print-only regression が pass する |
...
| DEV-DEFAULT-012 | `tools/ci/container_config.py`, `container_runtime.py`, `run_codex_in_repo_container.py`, `run_python_in_dockerfile.py`, `run_repo_program.py`, and AgentCanon-owned `tools/ci/codex-container-profiles.toml` implement the three-owner optional-config boundary; `documents/rule/repository-topic-clone.md` owns the ignored `workspace/` premise | focused tests reverse-map absent parent pack/profile/rules, source-relative default resolution, ignored managed HOME, and disabled credential projection; requiring a parent TOML, resolving the default profile from the parent, persisting HOME outside ignored `workspace/`, or enabling host `.git-credentials` by default is a drift blocker |
