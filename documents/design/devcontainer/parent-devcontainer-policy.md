...
切り離すことで、権限昇格、固定 GID への依存、host `/var/lib` への永続副作用を除去し、
...
| clause | operation | resulting state | completion evidence |
| --- | --- | --- | --- |
| DEV-GPU-001 | `gpu-admission.sh` から profile selector を `devcontainer up --config` に渡す | default selector/config/output/container と GPU-admission selector/config/output/container が分離する | selector の exact command、別 `dockerComposeFile`、`name` suffix、生成 Compose `name` の readback |
...
| DEV-GPU-004 | `devcontainer up` 完了後に同じ orchestrator が `devcontainer exec` で `finalize-shared-runtime.sh` を一度呼ぶ | provision receipt、bind mount、device/inode、namespace/mount、probe、container umask/group identity の不一致は fail-closed。default post-create/post-attach は finalize を呼ばない | finalize exit/readback、`shared-runtime-readback.json`、default/opt-in lifecycle scenario tests |
...
| rollback | GPU profile の失敗は default profile へ降格せず、profile container/project を停止する明示 cleanup routeへ返す。default profile へ host bootstrap を戻さない。rollback evidence は profile selector、host capability contract、別 Compose/project identity、receipt readback を含む |
...
| DEV-GPU-001 | `.devcontainer/gpu-admission/devcontainer.json` and `.devcontainer/gpu-admission.sh`: select a distinct profile config/output/project and call the profile lifecycle | default selector/output/container cannot be reused by profile; profile failure does not call default fallback | selector JSON, orchestrator command, output path, project suffix, lifecycle order |
...
| DEV-GPU-004 | `.devcontainer/gpu-admission.sh` then `.devcontainer/finalize-shared-runtime.sh`: run `devcontainer up` and explicit container finalize | finalize verifies the complete identity/bind/mount contract and publishes readback; any mismatch is non-zero | finalize receipt and exit evidence; default lifecycle remains without finalize |
