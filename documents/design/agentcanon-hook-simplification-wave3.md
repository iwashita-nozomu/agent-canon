...
- canonical design owner: [.codex/README.md](../../.codex/README.md) の Hook Context と `.codex/hooks/hook_dispatcher.py` の lifecycle contract
...
`hook_dispatcher.py` は `agent_canon_source_root.resolve_agent_canon_source_root` の既存 context を再利用し、`current_repository_root` を hook の active root として spool と report-target 解決に渡す。vendored/derived parent では hook の cwd が `vendor/agent-canon` submodule 内でも親 repo、standalone layout では AgentCanon repo が active root である。fixture 用の `AGENT_CANON_HOOK_SOURCE_ROOT` は既存の明示 override としてだけ使う。resolver failure は typed `failed` state とし、local spool capture は継続するが report projection capability を無効にする。`SOURCE_ROOT` はその fail-open spool root にだけ使い、report target fallback には使わない。

report target は次の順序で一度だけ解決する。

1. `AGENT_CANON_WORKFLOW_MONITOR_REPORT_DIR`（相対値は active root 基準、pointer containment とは別の明示 authority、既存 directory のみ）
1. active root の `reports/agents/.active_run` が指す既存 run bundle
...
| B4 | `branch_worktree_guard.py` and workflow docs → dispatcher | destructive Git intent, same-segment authority, redacted payload | the same `tools/runtime/authority/hook_safety.py`; policy wording remains [agents/canonical/CODEX_WORKFLOW.md](../../agents/canonical/CODEX_WORKFLOW.md) |
...
Fixtures are exact: `tests/fixtures/behavior_events/accepted.jsonl`, `duplicate.jsonl`, `malformed.jsonl`, `out_of_order.jsonl`, `escaping.jsonl`, and `oracle.json`. The historical parser's valid and malformed readback cases remain covered inline, while the behavior fixtures prove that the active parser never accepts legacy records as target records.
