...
| Canonical repository graph build, freshness, query, or context semantics are needed | [Dependency Manifest Design](./dependency-manifest-design.md), [Graph DSL Core](./structured-analysis/graph-dsl.md) | Defines the sole parser, parent-owned SQLite graph, completeness sets, and read-only consumer boundary. |
