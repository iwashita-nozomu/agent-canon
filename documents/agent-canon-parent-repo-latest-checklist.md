...
この checklist は、AgentCanon を `vendor/agent-canon/` に持つ親 repo で agent task を始める前に確認する最新状態 checklist です。
submodule が通常系ですが、legacy subtree / committed snapshot repo でも同じ親 repo 構造を確認します。
...
| --- | --- | --- | --- |
| `vendor/agent-canon/` | AgentCanon source checkout, submodule, subtree, or committed snapshot | AgentCanon | `git submodule status vendor/agent-canon` when submodule; otherwise `git log -- vendor/agent-canon` |
