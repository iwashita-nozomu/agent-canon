...
1. When feedback says the currently used skill is weak, shallow, late, or misrouted, target the active skill first. Repair both the discoverable runtime `SKILL.md` and the canonical `agents/skills/<skill>.md` owner, then update or verify the matching prompt eval. Memory-only capture does not close active-skill feedback unless the skill owner is explicitly out of scope with evidence.
