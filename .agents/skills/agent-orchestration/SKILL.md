...
1. For repo-changing tasks, keep parent-direct work as the default route. Use a multi-agent write-capable wave only when the user asks for it, or when independent replaceable targets can run in parallel and reduce the task. Repo-wide scope, multiple surfaces, long document sets, or shared runtime surfaces do not by themselves authorize a multi-agent wave. If runtime authorization or tool gates block a requested write-capable handoff, record the blocker and `fallback_exit_status` instead of expanding read-only analysis.
