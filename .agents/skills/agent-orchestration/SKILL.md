...
1. Before edit-path selection, parent-direct implementation, or write-capable subagent handoff, scale investigation to task risk. Broad surfaces, uncertain paths, and multi-agent handoffs need the `Pre-Edit Repository Investigation Packet` from `agents/COMMUNICATION_PROTOCOL.md`. Explicit-path work whose owner boundary, replaceable unit, validation route, and public impact boundary are already evidenced; Routine docs; Focused code; and typo/link/format-only work need the `Parent-Direct Context Note` from that document, including owner, path, request clause, reuse basis, design/OOP boundary, validation route, `llm_visible_context`, `local_tool_context`, and `durable_memory_refs`. Treat raw search hits, nearest editable files, or chat context alone as insufficient.
...
1. For repo-changing tasks, choose the implementation route by design/OOP boundary and ownership clarity rather than apparent size. Parent-direct work is appropriate when the responsibility boundary, owner path, reuse plan, and validation route are clear and the work stays inside one replaceable responsibility unit. Use a multi-agent write-capable wave only when the user asks for it, or when independent replaceable targets can run in parallel without expanding scope or weakening design responsibility. Repo-wide scope, multiple surfaces, long document sets, or shared runtime surfaces require an independent replaceable target before authorizing a multi-agent wave. If runtime authorization or tool gates block a requested write-capable handoff, record the blocker and `fallback_exit_status` instead of expanding read-only analysis.
...
1. Build the public skill set in this order:
   ...
   - add the task-shape skill set required by the current stage and contract:
     ...
     - owner-bounded fixes where the replaceable unit, validation route, and public-impact boundary are already evidenced; Routine docs; Focused code; typo/link/format-only edits; or explicit bounded-route requests -> `$small-change-routing`; keep `selected_runtime_skill_read` and `small_change_skill_read` evidence before patching, and keep contract-complete implementation as the completion basis. Apparent file count is only auxiliary context.
