...
1. For repo-changing tasks that span repo-wide scope, multiple surfaces, long document sets, shared runtime surfaces, or parallelizable independent targets, use a multi-agent write-capable wave as the default implementation route. Use parent-direct implementation only when a narrow Routine docs, Focused code, one-file, single-abstraction, or typo/link/format-only route is selected before editing with explicit risk class and validation route. If runtime authorization or tool gates block the write-capable handoff, record the blocker and `fallback_exit_status` in the run bundle instead of silently collapsing to parent-direct work.
