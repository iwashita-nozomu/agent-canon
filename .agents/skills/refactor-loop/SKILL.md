...
1. For dependency-guided structural duplicate cleanup, generate `priority_order`
   ...
   dependency-expanded repair batch/wave at a time, expand downstream affected
   files, reject responsibility-mixing root findings as `review_required`, and
   rerun the full scan after each batch before choosing the next batch.
...
1. Before launching a write-capable subagent, include a token-bounded handoff:
   every target object in the repair batch, allowed files, and an
   object-by-object repair intent.
