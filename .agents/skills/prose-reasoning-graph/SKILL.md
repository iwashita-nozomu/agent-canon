...
1. Treat tool results as bounded contracts: `ingest` / `ingest-set` returns `PROSE_REASONING_GRAPH_DB`, `analyze` mutates the DB and returns stats, `lint --out` returns active diagnostics with severity/rule/target/verification route, `integrate --out` returns operation count plus recursive verification routes, `skill-handoff --out` returns receiving-skill packets, and `project --out` is reserved for full graph inspection.
