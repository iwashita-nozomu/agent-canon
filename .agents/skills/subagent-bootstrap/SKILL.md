...
1. `close_agent` authority is runtime status `completed|errored|shutdown` or explicit user cancellation. Before closeout, close terminal run-local subagents as lifecycle cleanup and record `subagents_closed=yes` plus `Subagent Lifecycle Evidence` in `closeout_gate.md`. A nonterminal no-return instance records `subagents_closed=no` and `lifecycle_gate=pending`.
