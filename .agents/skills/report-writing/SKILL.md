...
1. Before writing report prose from a prose graph handoff, close `fix-now` findings at the DSL/projection stage: revise the structure contract, source-to-section map, or graph-backed rewrite packet, rerun graph diagnostics, and only draft report prose after the selected profile has no active findings.
