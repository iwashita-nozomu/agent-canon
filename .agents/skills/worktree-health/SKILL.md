...
1. Treat `git branch --show-current` and `git worktree list --porcelain` as diagnostics; branch/worktree creation is routed by `agents/canonical/CODEX_WORKFLOW.md` Branch Reuse Default and `tools/agent_tools/hook_safety.py`, while this skill checks only the recorded `branch_creation_reason=<reason>` or `worktree_creation_reason=<reason>` evidence.
