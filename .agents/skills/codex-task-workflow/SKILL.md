...
1. For repo-changing edits, read the full body of every selected runtime
   ...
   parent-direct routes; route owner-bounded edits through `$small-change-routing` and record small_change_skill_read evidence with skill
...
1. When the evidence shows a bounded owner, replaceable unit, targeted validation route, and no public behavior / schema expansion, or when the work is Routine docs, Focused code, or typo/link/format-only, add `$small-change-routing` before patching and keep targeted validation as the closeout route. File count is only auxiliary context.
