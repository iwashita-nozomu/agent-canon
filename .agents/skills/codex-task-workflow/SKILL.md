...
1. Keep validation on lightweight evidence first. Use static analysis, dependency checks, docs checks, route checks, and changed-file targeted tests before broader execution. Full CI, long test suites, benchmarks, experiments, GPU / CPU numerical runs, solver sweeps, and randomized large cases run from a task-linked approval note that ties the command to an active request clause, expected signal, runtime/resource budget, stop condition, and artifact path.
...
1. Treat task size, `Scoped Change Lite`, MVP, and thin slice labels as routing, wave, and validation-profile signals. Implementation behavior is derived from the request clauses, acceptance contract, implementation source packet, design trace, dependency-expanded scope, validation route, and review gate.
...
1. For fresh subagent launches, include the `Fresh Subagent Context Capsule` from `agents/COMMUNICATION_PROTOCOL.md` in the handoff. Pack exact paths and context artifacts instead of chat history, full transcripts, raw logs, full dashboards, or repo-root scope.
...
1. Validate with the cheapest relevant command first. Broader execution uses the task-linked approval note and records the signal that the lighter check left unresolved.
