...
1. If the user explicitly asks for subagent coding/implementation/patch/editing, do not route completion through read-only-only waves; launch write-capable `spark_worker`/`worker` (or equivalent write-capable subagent wave) after requirements, bounded allowed_paths, write scope, validation route, and `tool_rejection_preflight` are fixed.
...
1. Before implementation path selection, run or cite `agent-canon local-llm route-implementation-surface --request-file <request-or-design-question.txt> --format text` unless the approved design packet already fixes the owner, canonical paths, forbidden paths, and required checks. Use that compact route instead of rereading broad implementation files to decide where code, tool, skill, workflow, document, or runtime-instruction changes belong. If LocalLLM is unavailable, use the deterministic fallback `PRIMARY_PATHS` / `FORBIDDEN_PATHS` or record a blocker; do not select edit paths from raw search hits or chat impression.
