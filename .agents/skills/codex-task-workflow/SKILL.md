...
1. On a clean worktree, run `make agent-canon-ensure-latest` before planning or implementation. If repo-local dirty state blocks it, record why and rerun after commit or stash. If the dirty state is shared AgentCanon source, root shared surface, workflow, skill, subagent, or shared tool work, do not clean it by refreshing the pin; route it through `agents/workflows/agent-canon-pr-workflow.md` or `agents/workflows/derived-agent-canon-diff-workflow.md`, merge the AgentCanon PR or proposal first, then rerun `make agent-canon-ensure-latest` and `bash tools/sync_agent_canon.sh link-root` in the template / derived repo.
