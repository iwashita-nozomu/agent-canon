...
1. Use `python3 tools/agent_tools/route.py --prompt "<user request>" --format json` when a broad request needs deterministic public skill routing. Treat `ACTIVE_SKILLS` as current-wave skill guidance and `DEFERRED_SKILLS` as later wave triggers. `tools/bin/agent-canon local-llm route-skill ...` is the CLI compatibility mirror for the same catalog-backed route.
