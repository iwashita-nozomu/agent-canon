...
1. In parent mode, never edit `vendor/<module>` as a source branch. Use `python3 tools/agent_tools/dependency_module_change.py prepare --topic <topic> --module <module> --branch <branch> --owner-evidence <file>` for the exact topic membership, then hand the returned clone paths to VS Code standard multi-root operations (`Add Folder to Workspace` or `code --add <parent-clone> <dependency-clone>`).
