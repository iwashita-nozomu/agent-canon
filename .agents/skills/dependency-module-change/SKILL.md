...
1. In parent mode, never edit `vendor/<module>` as a source branch. Use `python3 tools/agent_tools/dependency_module_change.py prepare --topic <topic> --module <module> --branch <branch> --owner-evidence <file>` for the exact topic membership, then continue in the returned parent and source clone paths under the editor-independent `workspace/<topic-slug>/` contract.
