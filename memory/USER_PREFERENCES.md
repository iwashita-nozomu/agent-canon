...
- 2026-05-05 | レート制約が強い task では、repo inventory、tool drift survey、static validation planning、diff-local language review、design-traced bounded implementation slice を fresh subagent へ委譲し、parent は統合判断と最終責任に集中させる。具体的なモデル配分は 2026-07-11 の方針を正本とする。
  - source: chat
...
- 2026-07-11 | 親は Sol 系、子の通常 work / review は Luna 系を中心にし、mini helper と Spark は所有された限定用途へ分ける。reviewer を常設せず、該当 decision がある role だけを fresh context で起動し、token 効果は既存 session metrics で観測する。この項目は preference であり、正確な model / effort は `.codex/config.toml` と `.codex/agents/*.toml` が所有する。
  - source: chat
